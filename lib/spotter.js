...
const API_URL = process.env.SPOTTER_BASE_URL || 'https://api.exactspotter.com/v3';

if (!TOKEN_EXACT) {
  throw new Error('TOKEN_EXACT environment variable is not set. API calls will fail.');
}
...
async function fetchSpotter(endpoint, params) {
  ...
  const response = await fetch(url.toString(), {
    ...
    headers: {
      ...
      'Authorization': `Bearer ${TOKEN_EXACT}`,
    },
  });
  ...
  try {
    ...
    // Se o status for 200 mas o corpo for HTML, é um erro de servidor/proxy/firewall.
    if (responseBody.trim().startsWith('<!doctype html>')) {
      console.error(
        `[SPOTTER] Received HTML response with status ${response.status} for ${endpoint}. This often indicates a proxy error, firewall block, or missing/invalid authentication. Response text starts with: ${responseBody.substring(0, 50)}...`,
        e
      );
      // Retorna array vazio para não quebrar o build/app, mas loga o erro.
      return [];
    }
