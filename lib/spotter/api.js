...
async function fetchSpotter(endpoint, params) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      console.warn(`[SPOTTER] API request returned ${response.status} for ${endpoint}. Returning empty dataset. Response text: ${errorText.substring(0, 50)}...`);
      return [];
    }
  ...
  try {
    ...
    const errorText = await response.text(); // A resposta ainda pode ser lida aqui, pois response.ok é true
    console.error(
      `[SPOTTER] Failed to parse JSON for ${endpoint}. Response text starts with: ${errorText.substring(0, 50)}...`,
      e
    );
    throw new Error(
      `Failed to parse JSON for ${endpoint}. Status: ${response.status}. Response was not JSON.`
    );
  }
