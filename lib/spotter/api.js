...
async function fetchSpotter(endpoint, params) {
  ...
  try {
    ...
    // Não é necessário chamar response.text() aqui, pois o erro já será capturado se a resposta não for JSON.
    console.error(
      `[SPOTTER] Failed to parse JSON for ${endpoint}. Status: ${response.status}.`,
      e
    );
    throw new Error(
