...
const fallbackDataset = { funnel: [], sellers: [], presales: [], metrics: [], forecast: [] };
...
export async function loadSpotterDataset() {
  return safe(getSpotterDataset(), fallbackDataset);
}

export async function loadSpotterMetrics() {
  const rawData = await loadSpotterDataset();
