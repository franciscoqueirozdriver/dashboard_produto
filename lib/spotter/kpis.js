...
export function buildDataset({ leads = [], leadsSold = [], losts = [], productsDictionary = [] }) {
  ...
  // Lógica de extração de produto do lead removida.
  // Apenas o produto vendido será considerado.
  ...
  for (const lost of losts) {
    ...
    // Para leads perdidos, vamos assumir 'Sem produto informado'
    // já que o produto vendido não está disponível no payload de Losts.
    lostProducts.push({
      leadId: lost.leadId,
      date: lost.date,
...
export function getStatusByProduct(dataset) {
  ...
  // Com a mudança de foco para o "Produto Vendido", não temos mais o produto
  // associado a leads abertos de forma confiável.
  // A contagem de leads abertos por produto será removida,
  // e os leads abertos serão contados apenas no total (getSummary).
  // Se for necessário, o produto para leads abertos deve ser obtido de outra forma.
  // Por enquanto, vamos garantir que todos os produtos ganhos/perdidos sejam inicializados.
  for (const lead of dataset.leads) {
    const status = getLeadStatus(getLeadIdentifier(lead), dataset);
    if (status === 'open') {
      // Para manter a contagem de leads abertos, vamos associar a um produto genérico
      // ou ignorar a contagem por produto, focando apenas nos ganhos/perdidos.
      // Optando por ignorar a contagem por produto para leads abertos,
      // pois o produto do lead (produto do cliente) não é mais relevante.
      // A contagem total de leads abertos está em getSummary.
    }
  }
