...
export function toNumber(value) {
  ...
  // Se for uma string, tenta tratar formato brasileiro (ponto como milhar, vírgula como decimal)
  if (typeof value === 'string') {
    // Remove caracteres não numéricos (R$, espaços)
    strValue = strValue.replace(/[^0-9.,]/g, '');

    // 1. Remove separador de milhar (ponto) APENAS se houver vírgula, senão o ponto é decimal
    if (strValue.includes(',')) {
      strValue = strValue.replace(/\./g, '');
    }
    // 2. Substitui separador decimal (vírgula) por ponto
    strValue = strValue.replace(/,/g, '.');
  }
...
for (const sale of leadsSold) {
    ...
    if (Array.isArray(sale.product)) {
      // Verificar se algum produto tem finalValue definido
      const hasIndividualValues = sale.product.some(p => p?.finalValue !== undefined && p?.finalValue !== null);
      
      if (hasIndividualValues) {
        // Se pelo menos um produto tem valor individual, usar valores individuais com fallback para totalDealValue
        ...
        // Se nenhum produto tem valor individual, dividir totalDealValue igualmente
        const valuePerProduct = toNumber(sale.totalDealValue) / sale.product.length;
        for (const product of sale.product) {
          salesProducts.push({
            leadId: sale.leadId,
            saleDate: sale.saleDate,
            product: resolveProductName(product, dictionary),
            finalValue: valuePerProduct,
          });
        }
      }
    } else {
