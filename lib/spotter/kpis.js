...
function resolveLostReason(lost) {
  const candidates = [
    lost?.reason,
    lost?.discardReason,
...
function resolveLostSource(lost) {
  const candidates = [lost?.source, lost?.leadSource];
...
function resolveLostFunnel(lost) {
  const candidates = [lost?.funnel, lost?.value, lost?.funnelName];
...
function resolveLostProduct(lost, dictionary, recommendedProductsMap) {
  const recommendedProductId = lost?.leadId
    ? recommendedProductsMap.get(String(lost.leadId))
    : undefined;
  const recommendedName = recommendedProductId ? dictionary.get(recommendedProductId) : null;

  const candidates = [
    lost?.productName,
    lost?.product?.name,
    lost?.product?.title,
    lost?.product?.description,
    lost?.products?.[0]?.name,
    lost?.products?.[0]?.title,
    lost?.products?.[0]?.description,
    recommendedName,
  ];
...
export function buildDataset({ leads = [], leadsSold = [], losts = [], productsDictionary = [], recommendedProducts = [], products = [] }) {
  ...
  const normalizedLosts = [];

  for (const lost of losts) {
    if (!lost?.leadId) continue;

    const reason = resolveLostReason(lost);
    const source = resolveLostSource(lost);
    const productName = resolveLostProduct(lost, dictionary, recommendedProductsMap);
    const funnelName = resolveLostFunnel(lost);
    ...
    const normalizedLost = {
      ...
      source,
      productName,
      funnel: funnelName ?? lost?.funnel ?? null,
    };

    normalizedLosts.push(normalizedLost);
    lostByLead.set(String(lost.leadId), normalizedLost);
...
export function getSummary(dataset) {
  const { leads, leadsSold, soldByLead } = dataset;
  const totalSales = soldByLead.size;
